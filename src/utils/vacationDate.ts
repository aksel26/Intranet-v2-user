...
export const getWeekdaysBetweenDates = (dates: TDateRange, attendance: string | number) => {
  ...
  while (current.isBefore(end) || current.isSame(end, "day")) {
    ...
    if (dayOfWeek !== 0 && dayOfWeek !== 6) {
      result.push({ commuteDate: current.format("YYYY-MM-DD"), leaveTypeIdx: attendance });
    }

 ...
 * @param {string} attendance - 휴가 종류
 * @returns {string[]} YYYY-MM-DD 형식의 날짜 배열 (주말 제외)
 */

export const getWeekdaysBetweenDates = (dates: TDateRange, attendance: string) => {
  ...
  while (current.isBefore(end) || current.isSame(end, "day")) {
    ...
    if (dayOfWeek !== 0 && dayOfWeek !== 6) {
      result.push({ commuteDate: current.format("YYYY-MM-DD"), leaveType: attendance });
    }

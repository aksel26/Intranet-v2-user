...
const FileUpload = ({ opened, close, details }: any) => {
  ...
  const ButtonBranch = () => {
    if ((file && imagePreview) || (!file && !imagePreview)) {
      return (
        <>
          <Button variant="light" size="sm" fullWidth onClick={handleSave} disabled={!file ? true : false} leftSection={<Upload size={20} strokeWidth={1.5} />}>
            업로드
          </Button>
      ...
      return (
        <>
          <Button leftSection={<Download size={20} strokeWidth={1.5} />} variant="light" size="sm" fullWidth onClick={() => downloadImage(details?.imageUrl, details?.imageName)}>
            {details.imageName}
          </Button>
          <Button fullWidth size="sm" variant="light" color="red" leftSection={<X size={18} strokeWidth={1.2} />} onClick={deleteConfirmOpen}>
            삭제하기
          </Button>
        </>
      );
          ...
          <FileButton onChange={handleFileChange} accept="image/png,image/jpeg">
            {(props) => (
              <div className="relative w-full h-[300px] flex items-center justify-center group  transition-all duration-200 ease-in-out rounded-lg overflow-hidden cursor-pointer">
                <div>
                  <Image {...props} src={imagePreview} alt="Uploaded image" style={{ objectFit: "contain" }} className="group-hover:brightness-50 transition-all duration-200" />
                </div>

                <div className="opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 justify-center items-center flex flex-col absolute left-1/2 -translate-x-1/2">

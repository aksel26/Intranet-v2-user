...
function VacationConfirmModal({
  ...
  const submit = () => {
    ...
    temp.approverIdxs = submitInfo.confirmPerson.map((item: any) =>
      Number(item)
    );
        ...
        <Stack gap={"xs"}>
          ...
          <Stack gap={2}>
            {submitInfo?.leaveInfo?.map((info: any, key: number) => (
              <Group key={key}>
                <Text fz={"sm"} c={"dimmed"}>{`${dayjs(
                  info?.commuteDate
                ).format("YYYY-MM-DD (dd요일)")} `}</Text>
              </Group>
            ))}
            <Text c={"dimmed"} fz={"sm"}>
              승인자 :{" "}
              {confirmPerson?.map((person: any) => person.label).join(", ")}
            </Text>
          </Stack>
        </Stack>
      </Alert>

import { ActionIcon, Badge, Box, Button, Collapse, Divider, FileButton, Grid, Group, Modal, NumberInput, Paper, Radio, Select, Stack, Table, Text, Textarea, TextInput } from "@mantine/core";
  ...
  return (
    <Stack>
      <Group justify="space-between">
        ...
        <Button onClick={addRow} size="xs" leftSection={<Plus size={16} />} variant="light">
          행 추가
        </Button>
      </Group>
        ...
        <Table striped verticalSpacing={3} fz={10} horizontalSpacing={5}>
          ...
          <Table.Tbody>
            {data.map((row) => (
              <Table.Tr key={row.id}>
                <Table.Td>
                  <Select value={row.company} onChange={(e) => updateCell(row.id, "company", "React")} placeholder="이름 입력" size="xs" data={["React", "Angular", "Vue", "Svelte"]} />
                ...
                <Table.Td>
                  <NumberInput hideControls value={row.registeredCount} onChange={(val) => updateCell(row.id, "registeredCount", val)} placeholder="나이 입력" min={0} max={150} size="xs" />
                </Table.Td>
                <Table.Td>
                  <NumberInput hideControls value={row.attendanceCount} onChange={(val) => updateCell(row.id, "attendanceCount", val)} placeholder="나이 입력" min={0} max={150} size="xs" />
                </Table.Td>
                <Table.Td>
                  <TextInput value={row.format} onChange={(e) => updateCell(row.id, "format", e.target.value)} placeholder="이메일 입력" size="xs" />
                </Table.Td>
...
const CreateDrawer = ({ open, close }: any) => {
  ...
  return (
    <Modal opened={open} onClose={close} title="일정 등록" centered size="lg">
      ...
      <Stack>
        <Radio.Group name="favoriteFramework" label="유형 선택" withAsterisk styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }}>
          <Group mt={4}>
        ...
        <Collapse in={opened}>
          <TextInput styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} label="제목" placeholder="제목을 입력하세요." />
          <Radio.Group name="favoriteFramework" label="차량 사용 여부" styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }}>
            <Group mt={4}>
      ...
      <Modal opened={openedAddRow} onClose={addRowClose} title="시행정보 추가" centered>
        <Stack>
          <Stack gap={1}>
            ...
            <Grid>
              <Grid.Col span={4}>
                <Select label="그룹" size="xs" data={["LG", "SK", "ACG"]} styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} />
              </Grid.Col>
              <Grid.Col span={8}>
          ...
          <Stack gap={1}>
            ...
            <Grid>
              <Grid.Col span={6}>
                <Select label="검사" size="xs" data={["인적성", "인성", "적성", "기타"]} styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} />
              </Grid.Col>
              <Grid.Col span={6}>
                <Select label="구분" size="xs" data={["온라인", "오프라인"]} styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} />
              </Grid.Col>
            </Grid>
          </Stack>
          ...
          <Stack gap={1}>
            ...
            <Stack>
              <TextInput size="xs" label="감독관" placeholder="알바" styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} />
              <Select label="ACG 담당자" size="xs" data={["김정훈", "안정현"]} styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} />
              <TextInput placeholder="이름/연락처" size="xs" label="HR 담당자" styles={{ label: { fontSize: "var(--mantine-font-size-xs)", color: "var(--mantine-color-gray-5)" } }} />
            </Stack>
          </Stack>

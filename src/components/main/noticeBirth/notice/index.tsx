...
function Notice() {
  ...
  const { data, isLoading, isError } = useApiQuery(["notices", params], () =>
    ...
    <React.Fragment key={notice.noticeIdx}>
      {index + 1 > LIST_LIMIT ? null : (
        <Box
          ...
          onClick={() => goDetail(notice)}
          key={notice.noticeIdx}
        >
          <Indicator
            offset={-14}
            disabled={notice.isNew ? false : true}
            size={7}
            position="middle-end"
          >
  ...
  const renderContent = () => {
    ...
    if (isError)
      return <ErrorView>공지/일정을 불러오는 중 오류가 발생했어요.</ErrorView>;

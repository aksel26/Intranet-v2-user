...
export const checkIn = (values: any) =>
  apiClient.post(`/users/intranet/check-in`, values);

export const checkOut = (values: any) =>
  apiClient.put(`/users/intranet/check-out`, values);

export const leave = (values: any) =>
  postApi.post(`/users/intranet/leave`, values, {
...
export const deleteVacation = (values: any) =>
  postApi.delete(`/users/intranet/leave/${values.commuteIdx}`, {
    data: values,
  });

// 휴가 승인/반려하기 API
export const approveVacation = (values: any) =>
  postApi.patch(`/users/intranet/approval/${values.commuteIdx}`, {

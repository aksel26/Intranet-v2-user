...
export const getWelfares = ({ year, halfYear }: TWelfaresParams) =>
  getApi.get(`/users/welfares`, { params: { year, halfYear } });
export const getActivities = ({ year, halfYear }: TActivityParams) =>
  getApi.get(`/users/activities`, { params: { year, halfYear } });
...
export const getNotices = ({
  ...
  pageNo: number;
  perPage: number;
}) => getApi.get(`/users/notices`, { params: { pageNo, perPage } });
export const getNoticeDetail = ({ noticeIdx }: { noticeIdx: number }) =>
  getApi.get(`/users/notices/${noticeIdx}`, {
    params: { noticeIdx: noticeIdx },
  });
// 사용자 개인 휴가관리 요약정보 조회 API 개발
export const getAttendanceSummary = ({ year }: { year: number }) =>
  getApi.get(`/users/intranet/leave/stats`, { params: { year: year } });
...
export const getMyAttendance = (params: TMyAttendance) =>
  getApi.get(`/users/intranet/commute`, { params: params });

// 사용자 개인 휴가관리 상세정보 조회 API
export const getMyVacations = (params: TYearMonth) =>
  getApi.get(`/users/intranet/leave/detail`, { params: params });

// 사용자 이번달 업무시간 조회(차트)

export const getWorkHourStats = (params: TWorkHourStats) =>
  getApi.get(`/users/intranet/commute/work-hours`, { params: params });
...
export const getApprovals = (params: TApproval) =>
  getApi.get(`/users/intranet/approval`, { params: params });

// 결재 승인 내역 조회하기 API
export const getBirth = ({ month }: { month: string }) =>
  getApi.get(`/users/birth`, { params: { month: month } });

// 월별 전직원 휴가현황 조회 API
export const getAllAttendanceStaff = (params: TYearMonth) =>
  getApi.get(`/users/intranet/leave/all/calender`, { params: params });

// 사용자 공지사항 목록 조회 API
export const searchNotice = (params: TSearchNotice) =>
  getApi.get(`/users/notices`, { params: params });
